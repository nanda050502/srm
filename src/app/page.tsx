        ...
        <section className="px-8 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <MetricCard
              ...
              value={marquee.length}
              icon={<Crown className="h-7 w-7" />}
              onClick={() => router.push('/companies?category=marquee')}
            />
            <MetricCard
              label="Super Dream Companies"
              value={superDream.length}
              icon={<Rocket className="h-7 w-7" />}
              onClick={() => router.push('/companies?category=super-dream')}
            />
            <MetricCard
              label="Dream Companies"
              value={dream.length}
              icon={<Star className="h-7 w-7" />}
              onClick={() => router.push('/companies?category=dream')}
            />
            <MetricCard
              label="Regular Companies"
              value={regular.length}
              icon={<FileText className="h-7 w-7" />}
              onClick={() => router.push('/companies?category=regular')}
            />
          </div>
        ...
        <section className="bg-slate-100 px-8 py-12">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-slate-600 mb-6">
              Visit the Companies section for detailed profiles or compare hiring skill sets across recruiters.
            </p>
            <div className="flex gap-4 justify-center flex-wrap">
              ...
              <button
                onClick={() => router.push('/hiring-skill-set')}
...
const CompanyQuickCard: React.FC<CompanyQuickCardProps> = ({ company, onSelect }) => {
  ...
  const normalizeGrowth = (rate: string | number) => {
    ...
    if (typeof rate === 'number') return rate.toString();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <span className="text-sm font-bold text-green-600">{normalizeGrowth(company.yoy_growth_rate)}%</span>
